...
* `kCAMediaTimingFunctionLinear`옵션은 linear paced timing function함수를 생성한다. linear pacing은 거의 순간적으로 가속되는 것을 모델링 한 다음 목표에 도착할 때까지 크게 느려지지 않는다.
...
* 8장의 색성 전환 KeyFrameAnimation은 색상 간의 선형 페이싱으로 인해 약간 이상해 보였으므로 색상 사이의 전환이 자연스럽지 않게 발생하였다. 이를 보정하기 위해 kCAMediaTimingFunctionEaseIn 보다 적절한 easing function을 적용할 수 있다. 이 함수는 색인이 있는 전구가 실제처럼 작동하는 것처럼 레이어가 색상을 변경함에 따라 약간의 펄스 효과를 추가한다.
...
* 속성 애니메이션의 시작과 끝 값 사이에 임의의 중간 값을 가져오는 코드가 있으면 애니메이션을 원하는 만큼 많은 개별 프레임으로 분할하고 선형 키 프레임 애니메이션을 생성할 수 있다.
* 이제 예제를 해보자 !
