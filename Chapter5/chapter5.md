...
- CALayer에는 레이어의 뒷면을 그릴지 여부를 제어하는 doubleSided라는 속성이 있다. 기본값은 `true`이며 이 옵션을 `false`로 설정하면 레이어가 완전히 돌아갔을때 전혀 그려지지 않는다.(꾀 유용한 옵션인듯?)
...
```
- 위의 입방체는 이 각도에서 그냥 정사각형으로 보인다. 이를 위해 큐브 자체를 회전시킨다고 생각한다면 굉장히 성가신 일이다. 쉬운방법 없을까?
- 큐브 자체를 회전하여 해결하는것 보다는 카메라(즉 시점)의 위치를 바꾸는 것이다.
